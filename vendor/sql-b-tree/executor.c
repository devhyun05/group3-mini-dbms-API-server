...
SQLBT_THREAD_LOCAL TableCache open_tables[MAX_TABLES];
SQLBT_THREAD_LOCAL int open_table_count = 0;
static SQLBT_THREAD_LOCAL unsigned long long g_table_access_seq = 0;
static SQLBT_THREAD_LOCAL int g_executor_quiet = 0;
static SQLBT_THREAD_LOCAL DbResult *g_active_result = NULL;
static SQLBT_THREAD_LOCAL char *g_active_errbuf = NULL;
static SQLBT_THREAD_LOCAL size_t g_active_errbuf_size = 0;
static SQLBT_THREAD_LOCAL int g_api_capture_mode = 0;
static SQLBT_THREAD_LOCAL int g_api_result_failed = 0;
static SQLBT_THREAD_LOCAL int g_active_message_severity = 0;
...
static void remember_engine_message(const char *message);
static int executor_printf(const char *fmt, ...);
#define printf(...) executor_printf(__VA_ARGS__)
#define INFO_PRINTF(...) do { if (!g_executor_quiet) printf(__VA_ARGS__); } while (0)

static void clear_engine_message(void) {
    g_active_message_severity = 0;
    if (g_active_errbuf && g_active_errbuf_size > 0) {
        g_active_errbuf[0] = '\0';
    }
}
...
static void remember_engine_message(const char *message) {
    ...
    if (g_active_errbuf == NULL || g_active_errbuf_size == 0 || message == NULL) {
        return;
    }

    if (strncmp(src, "[error] ", strlen("[error] ")) == 0) {
        src += strlen("[error] ");
        severity = 3;
    } else if (strncmp(src, "[warning] ", strlen("[warning] ")) == 0) {
        src += strlen("[warning] ");
        severity = 2;
    } else if (strncmp(src, "[notice] ", strlen("[notice] ")) == 0) {
        src += strlen("[notice] ");
        severity = 2;
    } else if (strncmp(src, "[오류] ", strlen("[오류] ")) == 0) {
        src += strlen("[오류] ");
        severity = 3;
    }

    if (severity < g_active_message_severity) {
        return;
    }

    len = strcspn(src, "\r\n");
    if (len >= sizeof(cleaned)) {
        len = sizeof(cleaned) - 1;
    }
    memcpy(cleaned, src, len);
    cleaned[len] = '\0';

    strncpy(g_active_errbuf, cleaned, g_active_errbuf_size - 1);
    g_active_errbuf[g_active_errbuf_size - 1] = '\0';
    g_active_message_severity = severity;
}
...
static int executor_printf(const char *fmt, ...) {
    ...
    va_start(args, fmt);
    if (g_api_capture_mode) {
        char buffer[2048];
        va_list copy;

        va_copy(copy, args);
        written = vsnprintf(buffer, sizeof(buffer), fmt, copy);
        va_end(copy);
        remember_engine_message(buffer);
    } else {
...
static void set_default_error(const char *message) {
    if (g_active_errbuf == NULL || g_active_errbuf_size == 0 || message == NULL) {
        return;
    }
    if (g_active_errbuf[0] != '\0') {
        return;
    }
    strncpy(g_active_errbuf, message, g_active_errbuf_size - 1);
    g_active_errbuf[g_active_errbuf_size - 1] = '\0';
}

static int db_result_append_row(DbResult *result, const char *row_text) {
    char **grown;
    char *copy;

    if (result == NULL || row_text == NULL) {
        return 0;
    }

    if (result->row_count == result->row_capacity) {
        size_t next_capacity = result->row_capacity == 0 ? 8U : result->row_capacity * 2U;

        grown = (char **)realloc(result->rows, next_capacity * sizeof(char *));
        if (grown == NULL) {
            return 0;
        }
        result->rows = grown;
        result->row_capacity = next_capacity;
    }

    copy = dup_string(row_text);
    if (copy == NULL) {
        return 0;
    }

    result->rows[result->row_count++] = copy;
    return 1;
}
...
static int load_table_contents(TableCache *tc, const char *name, FILE *f) {
    ...
    if (tc->pk_idx != -1 && file_next_auto_id > tc->next_auto_id) {
        tc->next_auto_id = file_next_auto_id;
    }
...
static void emit_selected_row(const char *row, SelectExecContext *exec) {
    ...
    if (exec->select_all) {
        if (g_active_result != NULL) {
            if (!db_result_append_row(g_active_result, row)) {
                g_api_result_failed = 1;
                set_default_error("out of memory while building SELECT result");
            }
            return;
        }
        printf("%s\n", row);
    ...
    for (j = 0; j < exec->select_count; j++) {
        const char *value = fields[exec->select_idx[j]] ? fields[exec->select_idx[j]] : "";
        int written = snprintf(emitted_row + offset, sizeof(emitted_row) - offset,
                               "%s%s", (j > 0) ? "," : "", value);

        if (written < 0 || (size_t)written >= sizeof(emitted_row) - offset) {
            g_api_result_failed = 1;
            set_default_error("SELECT row is too long");
            return;
        }
    ...
    if (g_active_result != NULL) {
        if (!db_result_append_row(g_active_result, emitted_row)) {
            g_api_result_failed = 1;
            set_default_error("out of memory while building SELECT result");
        }
        return;
...
static int execute_select_internal(Statement *stmt, int emit_results, int emit_traces, int *matched_rows) {
    ...
    if (exec.emit_results && g_active_result == NULL) {
...
int db_execute_sql(const char *sql, DbResult *out, char *errbuf, size_t errbuf_size) {
    ...
    Statement stmt;
    int success = 0;
    int previous_quiet = g_executor_quiet;
    int matched_rows = 0;

    if (out == NULL) {
        if (errbuf != NULL && errbuf_size > 0U) {
            strncpy(errbuf, "output result buffer is required", errbuf_size - 1U);
    ...
    if (normalized_sql == NULL) {
        if (errbuf != NULL && errbuf_size > 0U) {
            strncpy(errbuf, "failed to copy SQL text", errbuf_size - 1U);
            errbuf[errbuf_size - 1U] = '\0';
        }
        return 0;
    }
    ...
    if (normalized_sql[0] == '\0') {
        if (errbuf != NULL && errbuf_size > 0U) {
            strncpy(errbuf, "SQL text is empty", errbuf_size - 1U);
            errbuf[errbuf_size - 1U] = '\0';
        }
        free(normalized_sql);
        return 0;
    ...
    g_api_capture_mode = 1;
    g_api_result_failed = 0;
    g_active_result = out;
    g_active_errbuf = errbuf;
    g_active_errbuf_size = errbuf_size;
    clear_engine_message();
    g_executor_quiet = 1;

    memset(&stmt, 0, sizeof(stmt));
    if (!parse_statement(normalized_sql, &stmt)) {
        if (stmt.type == STMT_UPDATE || stmt.type == STMT_DELETE) {
            set_default_error("only SELECT and INSERT are supported");
        } else {
            set_default_error("invalid SQL statement");
        }
        goto cleanup;
    }
    ...
    switch (stmt.type) {
        case STMT_SELECT:
            out->type = DB_RESULT_SELECT;
            if (!execute_select_internal(&stmt, 1, 0, &matched_rows) || g_api_result_failed) {
                set_default_error("SELECT failed");
                goto cleanup;
            }
            out->affected_rows = matched_rows;
            success = 1;
            break;
        case STMT_INSERT: {
            TableCache *tc = get_table(stmt.table_name);
            ...
            if (tc == NULL) {
                set_default_error("table could not be loaded");
                goto cleanup;
            }
            if (!insert_row_data(tc, stmt.row_data, 0, &inserted_id)) {
                set_default_error("INSERT failed");
                goto cleanup;
            }
            out->type = DB_RESULT_INSERT;
            out->affected_rows = 1;
            out->inserted_id = inserted_id;
            success = 1;
            break;
        }
        case STMT_UPDATE:
        case STMT_DELETE:
        case STMT_UNRECOGNIZED:
        default:
            set_default_error("only SELECT and INSERT are supported");
            break;
    }
...
cleanup:
    close_all_tables();
    open_table_count = 0;
    g_table_access_seq = 0;
    g_executor_quiet = previous_quiet;
    g_api_capture_mode = 0;
    g_api_result_failed = 0;
    g_active_result = NULL;
    g_active_errbuf = NULL;
    g_active_errbuf_size = 0;
    g_active_message_severity = 0;
    free(normalized_sql);
