...
서버는 멀티스레드 구조로 클라이언트 요청을 병렬 수신 및 처리하지만, **DB 엔진 내부는 전역 mutex에 의해 직렬화**되어 동시에 하나의 SQL만 실행된다. 
### 스레드 수를 코어 수로 제한한 이유
DB 실행 구간이 mutex로 직렬화되어 있기 때문에, 특정 시점에 실제로 SQL을 실행 중인 워커는 **항상 한 명뿐**이다. 나머지 워커는 mutex 해제를 기다리는 상태가 된다. 이 구조에서 워커를 코어 수 이상으로 늘려도: - DB 처리량은 **증가하지 않는다** (직렬화 구간이 병목). - 대기 워커가 늘어난 만큼 스케줄러의 **컨텍스트 스위칭 비용만 증가**한다. - 락 대기열이 길어져 **꼬리 지연(tail latency)도 악화**될 수 있다. 따라서 스레드 풀 크기는 하드웨어 병렬성의 상한인 **CPU 코어 수**로 고정했다. 이는 "DB 락을 잡은 워커 외의 다른 워커들이 네트워크 I/O, 응답 직렬화 등 mutex 바깥 작업을 동시에 수행할 수 있는 범위"와 정확히 일치한다. 
